...
if (isset($_SESSION['cat'])) {
    $statement = mysqli_query($conn,"
    SELECT * from product join brand on (product.BRAND_id = brand.brand_id) WHERE ADMIN_id = 3 AND product.BRAND_id = ".$_SESSION['cat']." ORDER BY product_name ASC LIMIT $start_from, $record_per_page; ");

} else {

$statement = mysqli_query($conn, "select * from product join brand on (product.BRAND_id = brand.brand_id) ORDER BY product_name ASC LIMIT $start_from, $record_per_page;");
}
        ...
        $output .= '<div class="col-lg-4">
                <div class="product product-item-background">
                    <div class="slick-product-img">
                        <a href="detail-'. str_replace(" ","-",$row['product_name']). '-'  .$row['product_id']. '.html">';

        $output .= '<img src=" ';
        if (empty($row['product_photo_link'])) {
...
/////////page number /////
    ...
    if (isset($_SESSION['cat'])) {
        $page_query = "SELECT * from product join brand on (product.BRAND_id = brand.brand_id) WHERE ADMIN_id = 3 AND product.BRAND_id = ".$_SESSION['cat']." ORDER BY product_name ASC;";
    }else {
        $page_query = "select * from product join brand on (product.BRAND_id = brand.brand_id) ORDER BY product_name ASC";
    }
  ...
  if ($page > 1 ) {
    $previous = $page -1 ;
    $render .= '
    <li class="page-item pagination_link" id="'.$previous.'"><a class="page-link" style="cursor:pointer;" tabindex="-1">Previous</a></li>';
  }
    for($i=1; $i<=$total_pages; $i++) {
        ...
        $render .= ' 
        <li class="page-item pagination_link '.$activePage.' " id="'.$i.'" ><a class="page-link" style="cursor:pointer;" >'.$i.'</a></li>
    ';
    }
    if ($page < $total_pages) {
        $page = $page +1;
        $render .= '
        <li class="page-item pagination_link" id ="'.$page.'"><a class="page-link" style="cursor:pointer;">Next</a></li>';
    }  
$render .= '</ul></nav> '; 
$render .= '</div><br /><br />';  
